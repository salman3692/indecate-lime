...
# -----------------------------
# Load the lime data
file_path = os.getenv('file_path', 'resuls_lime_combiprices_clean.csv')
...
def update_plots(selected_techs, cEE_min, cEE_max, cH2_min, cH2_max, cNG_min, cNG_max, cCO2_min, cCO2_max, cBioCH4_min, cBioCH4_max, cBiomass_min, cBiomass_max):
    ...
    fig = go.Figure(
        data=go.Parcoords(
            line=dict(color=[technology_reverse_mapping[c] for c in filtered_data['Technology']], colorscale='Viridis'),
            dimensions=[
                dict(range=[data_df['cEE'].min(), data_df['cEE'].max()], tickvals=[0.010, 0.025, 0.050, 0.075, 0.100, 0.125, 0.150, 0.175],label='Electricity<br>(€/MWh)', values=filtered_data['cEE']),
                dict(range=[data_df['cH2'].min(), data_df['cH2'].max()], tickvals=[0.010, 0.025, 0.050, 0.075, 0.100, 0.150, 0.20],label='Hydrogen<br>(€/MWh)', values=filtered_data['cH2']),
                dict(range=[data_df['cNG'].min(), data_df['cNG'].max()], tickvals=[0.010, 0.035, 0.055, 0.075, 0.1], label='NG<br>(€/MWh)', values=filtered_data['cNG']),
                dict(range=[data_df['cCO2'].min(), data_df['cCO2'].max()], tickvals=[0.075, 0.1, 0.2, 0.25],label='CO₂<br>(€/tCO₂)', values=filtered_data['cCO2']),
                dict(range=[data_df['cBioCH4'].min(), data_df['cBioCH4'].max()], tickvals=[0.03, 0.05, 0.07], label='Bio-CH₄<br>(€/MWh)', values=filtered_data['cBioCH4']),
                dict(range=[data_df['cBiomass'].min(), data_df['cBiomass'].max()], tickvals=[0.018, 0.030, 0.05], label='Biomass<br>(€/MWh)', values=filtered_data['cBiomass']),
